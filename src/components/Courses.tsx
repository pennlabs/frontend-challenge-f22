...
const Courses = ({ cart, setCart }: { cart: number[], setCart: Dispatch<SetStateAction<number[]>> }) => {
    const [alreadyTakenCourses, setAlreadyTakenCourses] = useState<number[]>([]); // stores course numbers, all courses are from the cis department
...
function Course({ course, handleAddToCart, cart }: { course: Course, handleAddToCart: (number: number) => void, cart: number[] }) {
    ...
    return (
        <div className='p-4 rounded-md opacity-60 hover:opacity-80 transition hover:border-stone-400 border-transparent border'>
            <div className="flex items-center">
                ...
                <button
                    className='w-10 h-10 ml-auto rounded-md p-2 bg-stone-200 hover:bg-stone-400 transition-colors'
